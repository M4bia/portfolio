...
def home(request):
    ...
    """
    projetos_destaque = Projeto.objects.filter(destaque=True, ativo=True)[:]
    return render(request, 'projetos/home.html', { 'projetos_destaque' : projetos_destaque})
    pass
...
def lista_projetos(request):
    ...
    categorias = Categoria.objects.all() #recupero tudo da base 
...
def detalhes_projeto(request, projeto_id):
    ...
    projetos_relacionados = Projeto.objects.filter( categoria = projeto.categoria, ativo=True) .exclude (id=projeto.id)[:3]
